...
public interface SLATrackingRepository extends JpaRepository<Sla, Integer> {
    ...
    // Find all active SLAs that are not paused and deadline is approaching
    ...
    // Resume SLA timer - simplified version
    @Modifying
    @Query("UPDATE Sla s SET s.isPaused = false, s.pauseTime = null WHERE s.ticket.id = :ticketId")
    void resumeSLA(@Param("ticketId") Integer ticketId);
    ...
    @Modifying
    @Query("UPDATE Sla s SET s.isPaused = false, s.deadline = :completionTime, s.pauseTime = null WHERE s.ticket.id = :ticketId")
